...
function showHelp {
   echo ""
   echo "This script is used to retrieve user activity on a system within a specific time period. Use the --details option"
   echo "to include detailed information about each login attempt, as well as peak concurrency events."
   echo ""
   echo "Usage:"
   echo ""
   ...
   echo "3. Fetch the list of users with current active sessions: ./analyzeUserActivity.sh --active"
   echo "4. Fetch the list of distinct users from today: ./analyzeUserActivity.sh --today"
   echo "5. Fetch the list of distinct users since the beginning of this week: ./analyzeUserActivity.sh --week"
   echo "6. Fetch the list of distinct users since January 1st, 2025: ./analyzeUserActivity.sh --since 2025-01-01 00:00:00"
   echo "7. Fetch the complete list of user logins from today: ./analyzeUserActivity.sh --today --details"
   ...
   echo " -t|--today = Displays the user sessions since the beginning of the current day."
   echo " -w|--week = Displays the user sessions since the beginning of the current week."
   echo " -l|--all = Displays the complete list of sessions.  Note the number of days surfaced here depends on how long the Viya audit service is configured to retain activity records for.  Depending on the number of records this may take a while to load."
   echo " -s|--since = Displays the user sessions since the specified date.  The date must be in the format of 'YYYY-mm-dd HH:MM:SS'."
   echo " -b|--before = Displays the user sessions before the specified date.  The date must be in the format of 'YYYY-mm-dd HH:MM:SS'."
   echo " -d|--details = Includes a detailed report of unique logon attempts."
...
function fetchSessionLogoff {
    local sessionId="$1"
    local filter="and(eq(type,security),eq(state,success),eq(action,delete),eq(properties.sessionSignature,'$sessionId'))"
    local auditUri="audit/activities?limit=$pageSize&start=$start&sortBy=timeStamp:ascending&filter=$filter"
...
function fetchAuditRecordsByDate {
    ...
    if [[ "$verbose" == "1" ]]
    then
        echo "Fetching list of sessions starting from '$sinceDate'"
    fi

    declare -A sessions
    auditOutput="audit-activity.json"
    ...
    filter="and(eq(type,security),eq(state,success),eq(action,create),gt(timeStamp,'$sinceDate')"
    if [[ "$beforeDate" != "" ]]
    then
    ...
    do
        ...
        while IFS="|" read -r user action timeStamp session; do
            ...
            then
                ...
                sessionEndDate=`fetchSessionLogoff "$session"`
    ...
    if [[ "$showDetails" != "1" ]]
    then
        echo "Total number of distinct users: ${#sessions[@]}"
        echo "Number of user sessions:"
        ...
        then
            ...
            echo "Peak concurrent user sessions: $peak (at $peakDisplay)"
...
function fetchSessionsCustomDate {
    echo ""

    if [[ "$since" != "" ]]
    then
        echo "User activity since '$since'..."
        sinceDate=$(date -d "${since}" +"%Y-%m-%dT%H:%M:%S.%3NZ")
    fi

    if [[ "$before" != "" ]]
    then
        echo "User activity before '$before'..."
...
function fetchSessionsFromThisWeek {
    echo ""
    echo "User activity from this week..."
...
function fetchSessionsFromToday {
    echo ""
    echo "User activity from today..."
...
#
# note: in 2024.09, the actions used by SASLogon for tracking login/logout activity changed. "SessionAuthenticationSession" has been
# replaced with "create", and "SessionDestroyed" has been replaced with "delete".
...
if [[ "$today" == "1" ]]
then
    fetchSessionsFromToday
fi
if [[ "$week" == "1" ]]
then
    fetchSessionsFromThisWeek
fi
